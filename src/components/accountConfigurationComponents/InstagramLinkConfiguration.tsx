...
const InstagramLinkConfiguration = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!value.startsWith("https://www.instagram.com")) {
      setIsError(true);
      setErrorMsg("Link must be a instagram link");
      return;
    }
    ...
    await axiosApiInstanceAuth
      .request({
        url: "accounts/me/update-instagram-link",
        method: "post",
        data: { newLink },
      })
      .then((res) => {
  ...
  useEffect(() => {
    if (user?.instagramLink && user?.instagramLink !== "")
      setNewLink(user?.instagramLink);
  }, []);
  ...
  return (
    ...
    >
      <p className="text-lg">Link your Instagram account.</p>
      ...
      <div className="w-full flex">
        ...
        {user?.instagramLink === newLink ||
        !newLink.startsWith("https://www.instagram.com") ? (
          <button
            type="submit"
...
export default InstagramLinkConfiguration;
