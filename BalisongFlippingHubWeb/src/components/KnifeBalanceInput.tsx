...
const KnifeBalanceInput = ({ setBalanceOnChange, setHasModulatedBalanceOnChange, parentBalanceValue, parentHasModulatedBalance }:params) => {
    const [balance, setBalance] = useState("3")
    const [hasModulatedBalance, setHasModulatedBalance] = useState(parentHasModulatedBalance)
    ...
    return (
        <div className="flex flex-col w-5/6 items-center gap-4">
            <div className="flex flex-col">
                <label>Does knife have a modular balance system?</label>
                ...
                <div className="flex justify-around">
                    <div className="flex flex-col gap-2">
                        <label>Yes</label>
                        {
                            hasModulatedBalance
                            ?
                            <input type="radio" defaultChecked name="yes/no" onChange={() => handleOnModulatedBalanceChange()} />
                            :
                            <input type="radio" name="yes/no" onChange={() => handleOnModulatedBalanceChange()} />
                        }
                    </div>
...

            {
                ...
                <div className="w-full flex flex-col items-center gap-2">
                    <span className="h-5 w-1 bg-white"></span>
                    <label>Where is the blanace point?</label>
                    <input 
                    type="range"
                    min="0"
                    max="6"
                    ...
                    <datalist id="balancePoint" className="flex flex-col justify-between w-full [writing-mode:vertical-lr] mt-4 text-lg">
