...
const FacebookLinkConfiguration = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!value.startsWith("https://www.facebook.com")) {
      setIsError(true);
      setErrorMsg("Link must be a facebooklink");
      return;
    }
    ...
    await axiosApiInstanceAuth
      .request({
        url: "accounts/me/update-facebook-link",
        method: "post",
        data: { newLink },
      })
      .then((res) => {
  ...
  useEffect(() => {
    if (user?.facebookLink && user?.facebookLink !== "")
      setNewLink(user?.facebookLink);
  }, []);
  ...
  return (
    ...
    >
      <p className="text-lg">Link your facebook account.</p>
      ...
      <div className="w-full flex">
        ...
        {user?.facebookLink === newLink ||
        !newLink.startsWith("https://www.facebook.com") ? (
          <button
            type="submit"
