...
const PersonalWebsiteLinkConfiguration = () => {
  ...
  const validateWebsite = (value: string) => {
    var hasValidEnding: boolean = false;
    var hasValidPrefix: boolean = false;
    ...
    if (hasValidEnding && hasValidPrefix) return true;
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!validateWebsite(value)) {
      setIsError(true);
      setErrorMsg("Link must be a valid website.");
      return;
    }
    ...
    await axiosApiInstanceAuth
      .request({
        url: "accounts/me/update-personal-website-link",
        method: "post",
        data: { newLink },
      })
      .then((res) => {
  ...
  useEffect(() => {
    if (user?.personalWebsiteLink && user?.personalWebsiteLink !== "")
      setNewLink(user?.personalWebsiteLink);
  }, []);
...
export default PersonalWebsiteLinkConfiguration;
