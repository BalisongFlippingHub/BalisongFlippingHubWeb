...
const DiscordLinkConfiguration = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!value.startsWith("https://www.discord.com")) {
      setIsError(true);
      setErrorMsg("Link must be a Discord link");
      return;
    }
    ...
    await axiosApiInstanceAuth
      .request({
        url: "accounts/me/update-discord-link",
        method: "post",
        data: { newLink },
      })
      .then((res) => {
  ...
  useEffect(() => {
    if (user?.discordLink && user?.discordLink !== "")
      setNewLink(user?.discordLink);
  }, []);
  ...
  return (
    ...
    >
      <p className="text-lg">Link your Discord account.</p>
      ...
      <div className="w-full flex">
        ...
        {user?.discordLink === newLink ||
        !newLink.startsWith("https://www.discord.com") ? (
          <button
            type="submit"
...
export default DiscordLinkConfiguration;
