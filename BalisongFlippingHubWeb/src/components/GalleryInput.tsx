...
const GalleryInput = ({ updateGalleryFiles, setStepManually, galleryFiles }:params ) => {
    ...
    const [selectedFiles, setSelectedFiles] = useState<File[] | null>(galleryFiles)
    ...
    const checkSelectedFileExistance = (file: File) => {
        if (selectedFiles !== null) {
            for (var i = 0; i < selectedFiles?.length!; i++) {
                if (file.size === selectedFiles[i].size && file.type === selectedFiles[i].type) return true
            }
        }
    ...
    return (
        <section className="flex flex-col pt-[58px] h-screen w-full lg:pl-[192px]">
            ...
            <div className="text-3xl font-bold w-full flex justify-between p-4">
                <button className="flex items-center gap-2 border p-2 hover:bg-shadow-green-offset" type="button" onClick={() => setStepManually("1")}>
                    <FontAwesomeIcon icon={faCircleArrowLeft} />
                    <h3 className="xsm:collapse xsm:absolute sm:static sm:visible">Edit Form</h3>
                </button>

                <h4 className="text-4xl">Gallery</h4>

                <button className="flex items-center gap-2 border p-2 hover:bg-shadow-green-offset" type="button" onClick={() => setStepManually("3")}>
                    <h3 className="xsm:collapse xsm:absolute sm:static sm:visible">Edit Post</h3>
            ...
            {/*Files Display*/}
            <div className="flex w-full lg:max-h-[70%] xsm:max-h-auto m-auto xsm:flex-col lg:flex-row">
                <input 
                    type="file"
                    ref={filesInputRef} 
                ...
                <div className="lg:w-1/2 border">
                    {
                        ...
                        ?
                        <img src={URL.createObjectURL(selectedFiles[currentIndex])} className="w-full h-full object-contain" />
                        :
                ...
                <div className="flex flex-col lg:w-1/2 border border">
...

                    {
                        ...
                        <div className="flex flex-wrap h-full">
                        {
                            selectedFiles?.map((file, i) => {
                                return (
                                    <div className="w-1/5 h-1/2 relative hover:cursor-pointer" key={i}>
                                        <img src={URL.createObjectURL(file)} className="h-full w-full object-cover" />
                                    
                                        <div className="bg-black absolute top-0 bottom-0 left-0 right-0 flex justify-center hover:opacity-75 opacity-0" onMouseOver={() => setCurrentIndex(i)}>
                                            <button type="button" className="w-full h-full" onClick={() => removeFile(i)}>Remove</button>
                                        </div>
                                    </div>
                                )
                            })
